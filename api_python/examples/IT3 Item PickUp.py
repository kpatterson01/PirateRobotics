...
def main():
    ...
    with utilities.DeviceConnection.createTcpConnection(args) as router:
        ...
        success &= basicMovement(base, .04, 0, 0, 0 , 12, 0)
        success &= basicMovement(base, .04, -.015, 0, 0 , 0, 0)
        #success &= basicMovement(base, 0, 0, .025, 0 , 0, 0)
        #success &= basicMovement(base, 0, -.025, 0, 0 , 0, 0)
        ...
        success &= basicMovement(base, 0, 0, .040, 0 , 0, 0)



        # # #GRIPPER COMMAND 
        example = GripperCommandExample(router)
        SendGripperCommands(example, 0.55)

        print("Item grabbed, bringing item back to home ")

        success &= basicMovement(base, 0, 0.025, 0, 0 , 0, 0)
        success &= example_move_to_home_position(base)
